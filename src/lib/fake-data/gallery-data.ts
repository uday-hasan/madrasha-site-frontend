import { GalleryImage, GalleryCategory } from "@/types/gallery";

export const fakeGalleryCategories: GalleryCategory[] = [
  { id: 1, name: "সকল", slug: "all" },
  { id: 2, name: "অনুষ্ঠান", slug: "events" },
  { id: 3, name: "শ্রেণিকক্ষ", slug: "classroom" },
  { id: 4, name: "খেলাধুলা", slug: "sports" },
  { id: 5, name: "অবকাঠামো", slug: "infrastructure" },
];

export const fakeGalleryImages: GalleryImage[] = [
  {
    id: 1,
    title: "হিফজ ক্লাস",
    description: "আমাদের শিক্ষার্থীদের হিফজ ক্লাস চলছে",
    videoUrl: "/videos/hifz_class.mp4",
    mediaType: "video",
    category: "classroom",
    date: "2024-12-15",
  },
  {
    id: 2,
    title: "ক্যালেন্ডার",
    description: "আমাদের বার্ষিক ক্যালেন্ডার ২০২৬",
    imageUrl: "/images/gallery/calender-2026.jpeg",
    mediaType: "image",
    category: "resource",
    date: "2024-11-20",
  },
  {
    id: 3,
    title: "শিক্ষার্থীদের পড়াশোনা",
    description: "আমাদের শিক্ষার্থীদের পড়াশোনা চলছে",
    videoUrl: "/videos/class-video-3.mp4",
    mediaType: "video",
    category: "classroom",
    date: "2024-11-10",
  },
  {
    id: 4,
    title: "শিক্ষার্থীদের শরীরচর্চা",
    description: "আমাদের শিক্ষার্থীদের শরীরচর্চা",
    imageUrl: "/videos/ex-video-1.mp4",
    mediaType: "video",
    category: "exercise",
    date: "2024-10-05",
  },
  {
    id: 5,
    title: "শিক্ষার্থীদের শরীরচর্চা",
    description: "আমাদের শিক্ষার্থীদের শরীরচর্চা",
    imageUrl: "/videos/ex-video-2.mp4",
    mediaType: "video",
    category: "exercise",
    date: "2024-10-05",
  },
  {
    id: 6,
    title: "ইফতার",
    description: "ইফতারে শিক্ষার্থীরা",
    imageUrl: "/images/gallery/iftar.jpeg",
    mediaType: "image",
    category: "events",
    date: "2024-09-15",
  },
];
